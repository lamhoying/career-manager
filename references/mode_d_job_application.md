...
| `07_career_identity` Layer 5 | Non-Positioning Statement（**读取时须剔除 Layer 2 已收录的方向**） |
...
Resume Identity Lock:
  ...
  禁止表达: [07 Layer 5 中未被 Layer 2 收录的条目]   # = Layer 5 ∖ Layer 2
  并行方向: [已被 Layer 2 收录、因而解禁的方向]      # 可为空
...
### 方向冲突探针（v2.20.0 · 与 G5 同构）

**触发**：本次 JD 的目标角色（或职能方向）**∈ Layer 5 非定位清单** 且 **∉ Layer 2 任一位置**。

**判定口径（v2.21.0）**：`∈ Layer 2` **一律按 `07` Layer 5 各条的「收录于 Layer 2」标注读取**
（标 `none` ⇒ ∉ Layer 2；标某 Layer 2 条目名 ⇒ ∈ Layer 2），**不做词元 / 子串比对**。
标注缺失或有疑 ⇒ **一律按 ∉ Layer 2 处理（即触发探针）** —— 存疑默认触发，与 default-open 同向。

**动作**：**停下并向用户显式呈现四选一 —— 默认不继续（禁止静默降级）**。

| 选项 | 含义 | 机制 |
|:--:|---|---|
| ① 照此生成 | 认可当前定位 | 身份层维持现状，继续 Step 5-10 |
| ② **纳入 Layer 2（并行）** | 该方向升为 Secondary / Emerging | 用户确认后走 **Mode B** 更新 `07`；**本次生成暂停** |
| ③ 单次例外（试点） | 仅本次 JD | **会话级**例外，不落盘 `07`，生成后即失效 |
| ④ 走 Mode B（转轨） | 认真转做该方向 | 转 Mode B 正式调整 `07` + 赛道文件 |

**禁止**：自动替用户选择｜自动改写 `07`｜静默按①继续。

**意图四态 → 机制映射**：

| 态 | 话语信号 | 机制 | 落 DNA |
|:--:|---|---|---|
| **I1 侦察** | 「看看 / 匹配度 / 分析」 | 只出 P1 报告（`01` 是分析非自述，Layer 5 **不参与**） | 无 |
| **I2 试点** | 「就投这一个试试」 | 单次例外（③） | 无（会话级） |
| **I3 并行** | 「也在考虑 X，但 Y 继续」 | 纳入 Layer 2（②）→ 禁令失效、主定位不动 | 轻改（+1 条） |
| **I4 转轨** | 「我要转做 X」 | Mode B 正式调整（④） | 正式改 |

> **默认开放（default-open）**：未被用户**显式排除**的方向 = 可投；**举证责任在「排除」侧**。
...
- 禁止从 JD 反推「我应该是什么身份」
- 07 Layer 5 中列出的原始岗位，**在其方向未被 Layer 2 收录时**，不得出现在简历的身份表达层
  （仅可作为事实信息在「公司·岗位」行出现；**已被 Layer 2 收录的方向不受此限**）

> **Fail-safe（v2.21.0 · F18）**：若 `07` Layer 2 **缺失、为空、或含 `[待声明]`** ⇒
> **停下询问用户**（复用方向冲突探针形态），**禁止从 Layer 1 / Timeline / JD 代填身份锚点**（违 `R01` / `R02`），
> **禁止静默降级**。
...
> **Step 8.x 编号说明（v2.18.0）**：本段步骤编号为 `8.5 / 8.6 / 8.8 / 8.9 / 8.10 / 8.12` —— **`8.7` 与 `8.11` 无对应步骤**（CHANGELOG 无删除记录，属历史跳号；**不补编号**，避免与历史留痕不符）。
...
- Headline/第一段中是否出现 07 Layer 5 禁止表达（**已剔除 Layer 2 收录方向**）→ 是则替换
